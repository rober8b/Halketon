...
export async function createDonation(input: DonationInput): Promise<Donation> {
  ...
  // 4. Simular confirmación de pago (en WB07 se integra Stellar aquí)
  ...
  // 5. Incrementar totales de la campaña (atómico vía SQL function)
  ...
  if (rpcError) {
    // No tiramos error — la donación ya está confirmada, solo falló el contador
    console.error('[donations] Error al incrementar totales:', rpcError);
  }

  return { ...donation, status: 'confirmed', payment_id: paymentId };
}
